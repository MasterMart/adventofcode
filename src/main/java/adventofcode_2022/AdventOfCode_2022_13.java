...
public class AdventOfCode_2022_13 {

    static String input = "[[],[],[[],10,[[7,0,1,1,10],9,6],[1,[4,9,1],6,[4,6,0]],[0,3,0]],[1,[10,[7,4,3,4],[]],[2,2],7,[[10],5,3]]]\n" +
    ...
    public static void main(String... args) {
        ...
        for (String s: all) {
            if (ordered.size() == 0) {
                ...
                int totallower = 0;
                for (String orderedstr: ordered) {
                    if (compareLists(orderedstr, s)) {
                        totallower ++;
                    }
                }
                ordered.add(totallower, s);
            }
        }
        int num1 = 0;
        int num2 = 0;
    ...
    public static Boolean compareLists(String first, String last) {
        List<String> firstlist = supersplit(first);
        List<String> lastlist = supersplit(last);
        int maxlength = Math.max(firstlist.size(), lastlist.size());
        for (int i = 0; i < maxlength; i++) {
            String firstelem;
            try {
                firstelem = firstlist.get(i);
            } catch (Exception e) {
                return true;
            }
            String lastelem;
            try {
                lastelem = lastlist.get(i);
            } catch (Exception e) {
                return false;
            }

            if (!firstelem.startsWith("[") && !lastelem.startsWith("[")) {
                Integer firstint = Integer.parseInt(firstelem);
                Integer lastint = Integer.parseInt(lastelem);
                if (firstint < lastint) {
                    return true;
                }
                if (lastint < firstint) {
            ...
            if (firstelem.startsWith("[") && !lastelem.startsWith("[")) {
                Boolean result = compareLists(firstelem, "[" + lastelem + "]");
                if (result != null) {
                    return result;
                } else {
                    continue;
    ...
    public static List<String> supersplit(String s) {
        ...
        for (int i = 1; i < s.length() - 1; i++) {
            ...
            if (j.equals("[")) {
                String flist = "[" + AdventOfCodeUtil.determineTextBetweenBrackets(s, i) + "]";
                i += flist.length();
                splitted.add(flist);
            } else {
                splitted.add(AdventOfCodeUtil.getFirstNumberOfString(s.substring(i)).toString());
            }
        }
        return splitted;
    }
