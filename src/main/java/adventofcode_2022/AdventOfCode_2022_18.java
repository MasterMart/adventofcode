...
public class AdventOfCode_2022_18 {
	...
	static int maxy = -10000;
	static int maxz = -10000;
	static int minx = 1000;
	static int miny = 1111;
	static int minz = 1111;

	static int totalWallInside = 0;
	static int totalWalls = 0;
	static Set<Coordinate> alreadyProcessed = new HashSet<>();
	static Set<Coordinate> processedThisIter = new HashSet<>();
	...
	public static void main(String... args) throws Exception {
		...
		for (int i = 0; i < maxx; i++) {
			for (int j = 0; j < maxy; j++) {
				for (int k = 0; k < maxz; k++) {
					...
					if (alreadyProcessed.contains(coordinate)) {
						continue;
					}
					...
					if (coordinateList.contains(coordinate)) {
						...
						try {
							process(coordinateList, coordinate);
							totalWalls += totalWallInside;
							alreadyProcessed.addAll(processedThisIter);
							totalWallInside = 0;
							processedThisIter = new HashSet<>();
						} catch (Exception e) {
							alreadyProcessed.addAll(processedThisIter);
							processedThisIter = new HashSet<>();
							totalWallInside = 0;
		...
		System.out.println(4504 - totalWalls);
		throw new Exception();
	}

	private static void process(List<Coordinate> coordinateList, Coordinate coordinate) throws Exception {
		processedThisIter.add(coordinate);
		List<Coordinate> adjacent = findAdjacent(coordinate);
		for (Coordinate adjacentCoor: adjacent) {

			if (processedThisIter.contains(adjacentCoor)) {
				continue;
			}

			if (alreadyProcessed.contains(adjacentCoor)) {
				throw new Exception();
			}

			if (coordinateList.contains(adjacentCoor)) {
				totalWallInside += 1;
				continue;
			}

			if (adjacentCoor.y >= maxy || adjacentCoor.z >= maxz || adjacentCoor.x >= maxx
					|| adjacentCoor.x <= minx || adjacentCoor.y <= miny || adjacentCoor.z <= minz) {
				throw new Exception();
			}

			process(coordinateList, adjacentCoor);

		}
	}

	public static List<Coordinate> findAdjacent(Coordinate coordinate) {
		List<Coordinate> coordinates = new ArrayList<>();
		Coordinate coordinate1 = new Coordinate(coordinate.x, coordinate.y, coordinate.z - 1);
		if (coordinate1.y <= maxy && coordinate1.z <= maxz && coordinate1.x <= maxx
				&& coordinate1.x >= minx && coordinate1.y >= miny && coordinate1.z >= minz) {
			coordinates.add(coordinate1);
		}
		Coordinate coordinate2 = new Coordinate(coordinate.x, coordinate.y, coordinate.z + 1);
		if (coordinate2.y <= maxy && coordinate2.z <= maxz && coordinate2.x <= maxx
				&& coordinate2.x >= minx && coordinate2.y >= miny && coordinate2.z >= minz) {
			coordinates.add(coordinate2);
		}
		Coordinate coordinate3 = new Coordinate(coordinate.x, coordinate.y - 1, coordinate.z);
		if (coordinate3.y <= maxy && coordinate3.z <= maxz && coordinate3.x <= maxx
				&& coordinate3.x >= minx && coordinate3.y >= miny && coordinate3.z >= minz) {
			coordinates.add(coordinate3);
		}
		Coordinate coordinate4 = new Coordinate(coordinate.x, coordinate.y + 1, coordinate.z);
		if (coordinate4.y <= maxy && coordinate4.z <= maxz && coordinate4.x <= maxx
				&& coordinate4.x >= minx && coordinate4.y >= miny && coordinate4.z >= minz) {
			coordinates.add(coordinate4);
		}
		Coordinate coordinate5 = new Coordinate(coordinate.x + 1, coordinate.y, coordinate.z);
		if (coordinate5.y <= maxy && coordinate5.z <= maxz && coordinate5.x <= maxx
				&& coordinate5.x >= minx && coordinate5.y >= miny && coordinate5.z >= minz) {
			coordinates.add(coordinate5);
		}
		Coordinate coordinate6 = new Coordinate(coordinate.x - 1, coordinate.y, coordinate.z);
		if (coordinate6.y <= maxy && coordinate6.z <= maxz && coordinate6.x <= maxx
				&& coordinate6.x >= minx && coordinate6.y >= miny && coordinate6.z >= minz) {
