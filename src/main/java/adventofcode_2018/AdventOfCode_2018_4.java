...
public class AdventOfCode_2018_4 {

    public static void main(String... args) {
        String input = "[1518-08-06 00:13] falls asleep\n" +
        ...
        List<InformationLine> informationLineList = new ArrayList<>();


        for (String s : input.split("\\n")) {
            InformationLine informationLine = new InformationLine();
            informationLine.year = Integer.parseInt(s.substring(1, 5));
            informationLine.month = Integer.parseInt(s.substring(6, 8));
            informationLine.day = Integer.parseInt(s.substring(9, 11));
            informationLine.hour = Integer.parseInt(s.substring(12, 14));
            informationLine.minute = Integer.parseInt(s.substring(15, 17));
            informationLine.textString = s.substring(s.indexOf("]") + 2);
            informationLineList.add(informationLine);
        }
        Collections.sort(informationLineList);

        Map<String, SleepStats> map = new HashMap<>();
        String lastGuardInMem = "";
        for (int i = 0; i < informationLineList.size() - 1; i++) {
            InformationLine informationLine = informationLineList.get(i);
            if (informationLine.textString.contains("Guard")) {
                lastGuardInMem = informationLine.textString.substring(0, informationLine.textString.indexOf(" b"));
                if (!map.containsKey(informationLine.textString.substring(0, informationLine.textString.indexOf(" b")))) {
                    SleepStats sleepStats = new SleepStats();
                    sleepStats.sleeptime = 0;
                    sleepStats.sleepminutes = new ArrayList<>();
                    map.put(informationLine.textString.substring(0, informationLine.textString.indexOf(" b")), sleepStats);
                }
            } else if (informationLine.textString.contains("wakes")) {
                int sleeptime = informationLine.calculateTime(informationLineList.get(i - 1));
                SleepStats sleepStats = map.get(lastGuardInMem);
                sleepStats.sleeptime = sleeptime + map.get(lastGuardInMem).sleeptime;
                sleepStats.sleepminutes.addAll(makeList(informationLineList.get(i - 1).minute, informationLine.minute));
                map.put(lastGuardInMem, sleepStats);
            }
        }
        ...
        for (String s : map.keySet()) {
            ...
            for (int i : map.get(s).sleepminutes) {
                if (maps.containsKey(i)) {
                    int sleepminute = maps.get(i);

                    maps.put(i, sleepminute + 1);
                } else {
                    maps.put(i, 0);
                }
            }
        ...
        for (String s: newmap.keySet()) {
            for (int i : newmap.get(s).keySet()) {
                if (newmap.get(s).get(i) > amountmins) {
                    minutemax = i;
                    amountmins = newmap.get(s).get(i);
                    guardmax = s;
                }
            }
        }

        System.out.println(minutemax +"  " + guardmax);
    }

    private static List<Integer> makeList (int begin, int end) {
    ...
    public static class SleepStats {
        ...
        public List<Integer> sleepminutes;
    }

    public static class InformationLine implements Comparable<InformationLine> {
        public int year;
        public int month;
        public int day;
        public int hour;
        ...
        public int calculateTime(InformationLine informationLine) {
            return Math.abs(minute - informationLine.minute);
        }

        @Override
        public int compareTo(InformationLine informationLine) {
            if (year > informationLine.year) {
                return 1;
            }
            if (year < informationLine.year) {
                return -1;
            ...
            if (year == informationLine.year) {
                if (month > informationLine.month) {
                    return 1;
                }
                if (month < informationLine.month) {
                    return -1;
                ...
                if (month == informationLine.month) {
                    ...
                    if (day == informationLine.day) {
                        ...
                        if (hour == informationLine.hour) {
                            if (minute > informationLine.minute) {
                                return 1;
                            }
                            if (minute < informationLine.minute) {
                                return -1;
